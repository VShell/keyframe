...
impl<R: Read + Unpin> Read for ChunkedDecoder<R> {
    ...
    fn poll_read(
        ...
        loop {
            if n.end >= buffer.len() {
                eprintln!("reallocating, len is {} and end is {}", buffer.len(), n.end);
                if buffer.len() + 1024 <= MAX_CAPACITY {
                    buffer.realloc(buffer.len() + 1024);
                    eprintln!("new buffer len is {}", buffer.len());
                } else {
                    this.buffer = buffer;
                    this.current = n;
            ...
            if needs_read {
                ...
                match (bytes_read, &this.state) {
                    ...
                    (0, _) => {
                        // Unexpected end
                        // TODO: do something?
                        eprintln!("unexpected end 2 - buffer size {}, end {}, capacity {}, n.len() {}", buffer.len(), n.end, buffer.capacity(), n.len());
